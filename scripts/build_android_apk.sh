...
# --no-build-cache forces Dart recompilation so main.dart patches take effect.
# (Gradle incremental cache would otherwise reuse stale libapp.so.)
flutter build apk --split-per-abi --no-version-check --no-build-cache --suppress-analytics
FLUTTER_RC=$?
set -e
