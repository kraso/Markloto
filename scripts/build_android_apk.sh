...
# Force a CLEAN rebuild so main.dart patches take effect.
# flutter's incremental build caches Dart compilation in .dart_tool/ and
# build/app/intermediates/.  Deleting those + touching the source is not
# enough — Gradle/Dart still reuses the cached kernel -> stale libapp.so.
# We use 'flutter clean' to wipe the build cache entirely, then regenerate
# flutter_native_splash and flutter_launcher_icons resources (which are needed
# but are wiped by flutter clean), then do a full compile with patched source.
echo "  -> Running flutter clean (forza recompilación completa de Dart)..."
flutter clean --suppress-analytics 2>/dev/null || true

# Regenerar recursos de splash e iconos (flutter clean los borra)
echo "  -> Regenerando flutter_native_splash resources..."
flutter pub run flutter_native_splash:create --suppress-analytics 2>/dev/null || true
echo "  -> Regenerando flutter_launcher_icons..."
flutter pub run flutter_launcher_icons:main --suppress-analytics 2>/dev/null || true

# Re-apply NormalTheme sed AFTER flutter_native_splash regenerates styles.xml
for styles in \
  "$FLUTTER_DIR/android/app/src/main/res/values/styles.xml" \
  "$FLUTTER_DIR/android/app/src/main/res/values-night/styles.xml"; do
  if [[ -f "$styles" ]]; then
...
# touch main.dart to ensure Gradle sees it as modified
touch "$MAIN_DART"
